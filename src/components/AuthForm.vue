...
<template>
  <div>
    ...
    <div class="grid md:flex space-x-2 gap-2">

      <button @click="gitHubAuth"
        class="flex cursor-pointer items-center bg-white border border-gray-300 truncate rounded-lg shadow-md max-w-xs w-full px-6 py-2 text-sm font-medium text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
        <svg class="h-6 w-6 mr-2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
          ...
          <g id="team-collaboration/version-control/github" stroke="none" stroke-width="1" fill="none"
            ...
            <g id="container" transform="translate(2.000000, 2.000000)" fill-rule="nonzero">
              <rect id="mask" stroke="#000000" stroke-width="2" fill="#000000" x="-1" y="-1" width="71" height="71"
                rx="14">
      ...
      <button @click="googleAuth"
        class="flex cursor-pointer items-center bg-white border border-gray-300 rounded-lg shadow-md max-w-xs w-full truncate px-6 py-2 text-sm font-medium text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
        <svg class="h-6 w-6 mr-2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
      ...
      <!-- <VButton :loading="gitHubLoading" :disabled="loading" type="button" buttonColor="pink"
        class="flex items-center justify-center bg-black" @click="gitHubAuth">
        <i-mdi-github class="h-5 w-5" />
      </VButton>
      <VButton :loading="googleLoading" :disabled="loading" type="button" buttonColor="green"
        class="flex items-center justify-center bg-[#EA4335]" @click="googleAuth">
        <i-mdi-google class="h-5 w-5" />
      </VButton>
      <VButton :loading="twitterLoading" :disabled="loading" type="button" buttonColor="red"
        class="flex items-center justify-center bg-[#1DA1F2]" @click="twitterAuth">
        <i-mdi-twitter class="h-5 w-5" />
      </VButton>
      <VButton :loading="facebookLoading" :disabled="loading" type="button" buttonColor="cyan"
